...
const LOADER_MIN_ACTIVE_TIME = 7.5; 

const COUNTER_ANIMATION_TARGET_DURATION = Math.max(7.0, LOADER_MIN_ACTIVE_TIME - 0.5); 

let pageHasLoaded = false;

let counterAnimationState = { value: 0 };
const counterTween = gsap.to(counterAnimationState, {
    value: 100,
    duration: COUNTER_ANIMATION_TARGET_DURATION,
    ease: "power1.out", 
    onUpdate: () => {
        counterElement.textContent = Math.floor(counterAnimationState.value) + "%";
    },
    onComplete: () => {
        if (Math.floor(counterAnimationState.value) < 100) { 
...
window.addEventListener('load', () => {
    ...
    if (counterAnimationState.value < 100) {
        counterTween.kill(); 

        gsap.to(counterAnimationState, {
            value: 100,
            duration: 0.3, 
            ease: "power1.out",
            onUpdate: () => {
...
masterTl
    ...
    .to(".block", { 
        ...
        height: "0px", 
        ease: "power4.inOut",
        onComplete: () => gsap.set(".block", { display: "none" }) 
    }, 6) 
    ...
    }, LOADER_MIN_ACTIVE_TIME) 

    .to(".hero", { 
        duration: 2, 
        scale: 1, 
        ease: "power4.inOut" 
    }, LOADER_MIN_ACTIVE_TIME + 0.5); 
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (menuToggle && menuContainer) {
        ...
        function shuffleChars(el) {
            if (!el || el.dataset.originalText == null) return;
            ...
            chars.forEach((char, i) => {
                const shuffleTime = 250; // total shuffle duration per char
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        // Reveal original letter with delay
                        setTimeout(() => {
        ...
        function applyShuffleHover(selector) {
            const elements = document.querySelectorAll(selector);
            elements.forEach(el => {
                // Store original text and split once
                el.dataset.originalText = el.textContent;

                if (!el.classList.contains("chars-split")) {
                    new SplitType(el, { types: "chars" });
                    el.classList.add("chars-split");
                }

                el.addEventListener("mouseenter", () => shuffleChars(el));
            });
        }


        applyShuffleHover(".wow");
        applyShuffleHover(".text");
        applyShuffleHover(".skill-pill, .footer-large-text ,.light-text ,.bold-text");
...
if (window.innerWidth <= 900) {
    const teamImages = document.querySelectorAll(".profile-images .img");
    const teamNames = document.querySelectorAll(".profile-names .name h1");

    teamImages.forEach((imgEl) => {
        imgEl.addEventListener("click", () => {
            const index = parseInt(imgEl.dataset.index, 10);
            if (!index || !teamNames[index]) return;

            // Hide all names
            gsap.set(teamNames, { autoAlpha: 0, y: "100%" });

            // Show selected name
            gsap.to(teamNames[index], {
                autoAlpha: 1,
                y: "0%",
                duration: 0.5,
                ease: "power2.out"
...
document.addEventListener("DOMContentLoaded", () => {
    // Your existing code initializing Lenis, menus, etc...
    // --- MOBILE FALLBACK (Reset styles, disable ScrollTrigger, disable canvas) ---
    if (isMobile()) {
        // Reset Work Section layout
        const workCards = document.querySelector(".work .cards");
        if (workCards) {
            gsap.set(workCards, { clearProps: "all" });
            workCards.style.width = "100%";
            workCards.style.flexDirection = "column";
            workCards.style.paddingLeft = "0";
        }

        // Reset Review Section layout
        const reviewsContainer = document.querySelector(".review-cards-container");
        if (reviewsContainer) {
            gsap.set(reviewsContainer, { clearProps: "all" });
            reviewsContainer.style.flexDirection = "column";
        }

        // Hide canvases for reviews (if present)
        document.querySelectorAll("canvas.outline-layer, canvas.fill-layer").forEach(canvas => {
    ...
    // --- TEAM SECTION MOBILE TAP HANDLER ---
    if (isMobile()) {
        const teamImages = document.querySelectorAll(".profile-images .img");
        const teamNames = document.querySelectorAll(".profile-names .name h1");
    ...
    // --- Resize handler to re-evaluate layout on screen change ---
    window.addEventListener("resize", () => {
        location.reload(); // Easiest way to reload mobile/desktop logic safely
    });

    gsap.set(".skill-pill", { opacity: 0, filter: "blur(8px)", y: 20 });
